...
struct RepoWatcherWidgetEntryView : View {
    ...
    var daysSinceLastActivity: Int {
        calculateTimeSinceLastActivity(from: entry.repo.pushedAt)
    }
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading) {
                ...
                HStack {
                    StatLabel(value: entry.repo.watchers,
                              systemImageName: "star.fill")
                    StatLabel(value: entry.repo.forks,
    ...
    func calculateTimeSinceLastActivity(from dateString: String) -> Int {
        let lastActivityDate = formatter.date(from: dateString) ?? .now
        let daysSinceActivity = Calendar.current.dateComponents([.day], from: lastActivityDate, to: .now).day ?? 0
...
struct RepoWatcherWidget_Previews: PreviewProvider {
    static var previews: some View {
        RepoWatcherWidgetEntryView(entry: RepoEntry(date: Date(),
                                                    repo: Repository.placeholder, avatarImageData: Data()))
